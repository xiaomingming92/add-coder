...
| `Plan` / `plan` | **二段式**: ①先读 `.vscode/plans/index.md` 按 planName 匹配路径（P0 优先）；②无匹配才全局 glob 搜索 `*-plan-v*.md`。**禁止跳过 index.md 直接 glob**。**模板选择**：≤3 文件、无新模块/架构 → `simple-plan-template.md`（Tasks+Handoff 融合在 Plan 体内，无需独立 spec/handoff 文件）；多模块/跨系统/含架构 → `standard-plan-template.md` | 🔴 P0 |
| `Spec` / `spec` | 查 `.vscode/specs/{name}/spec.md` | 🔴 P0 |
| `Tasks` / `tasks` | 查 `.vscode/specs/{name}/tasks.md` | 🟡 P1 |
| `Checklist` / `checklist` | 查 `.vscode/specs/{name}/checklist.md` | 🟡 P1 |
| `Review` / `review` | **二段式**: ①先读 `.vscode/plans/index.md` 定位 plan → 查其关联 Review；②无匹配才全局 glob `*-review*.md` | 🔴 P0 |
| `review-implementation` | 查 `.vscode/reviews/` 下 `*-review-implementation*.md` | 🟡 P1 |
| `review-runtime` | 查 `.vscode/reviews/` 下 `*-review-runtime*.md` | 🟡 P1 |
| `Handoff` / `handoff` / `交接` | **二段式**: ①先读 `.vscode/plans/index.md` 按 planName 匹配路径 → 定位 handoff；②无匹配才全局 glob `*-handoff*.md` | 🔴 P0 |
| `add-route` / `执行路线图` | **二段式**: ①先读 `.vscode/plans/index.md` 定位 plan → 查其 add-route；②调 `check_add_route_status`；③无匹配才全局 glob | 🔴 P0 |
| `devlog` / `开发日志` | **双触发**: ①用户说"devlog 记录下"→ 立即写；②Step 8 收敛通过 → 自动写（无需提醒）| 🔴 P0 |
| `index.md` / `Plan索引` | 读 `.vscode/plans/index.md`（匹配依赖优先）。**当用户问及任何 doc 类型（Plan/Review/Handoff/add-route）但未给明确路径时，此条优先于上述所有 doc 类型触发词执行** | 🔴 P0 |
| `gateway.md` / `gateway报告` / `运行时报告` | 读 `.vscode/reports/add-coder-runtime-report/gateway.md` | 🟡 P1 |
| `report-handoff` / `report交接` | 读 `.vscode/templates/report-handoff-template.md` | 🟡 P1 |
...
② 更新 handoff → Write(".vscode/plans/{date}/xxx-handoff-v1.md", "handoff 模板内容")
...
**正确输出**：
```
LLM: "先查索引 → Read('.vscode/plans/index.md') 查找相关 Plan 条目"
...
LLM 词汇匹配: "plan" 命中 → 第 19 行二段式:
  ① Read('.vscode/plans/index.md') — 搜索 "three-tier-reasoning-graph"
...
> **维护**: 单一真值源，被 `.vscode/rules/project_rules.md` 和 `AGENTS.md` 引用。
...
| P0 | `Plan` / `plan` | **二段式**: ①先读 `.vscode/plans/index.md` 按 planName 匹配路径；②无匹配才全局 glob。**index.md 优先** |
| P0 | `Spec` / `spec` | 查 `.vscode/specs/{name}/spec.md` |
| P1 | `Tasks` / `tasks` | 查 `.vscode/specs/{name}/tasks.md` |
| P1 | `Checklist` / `checklist` | 查 `.vscode/specs/{name}/checklist.md` |
| P0 | `Review` / `review` | **二段式**: ①先读 `.vscode/plans/index.md` 定位 plan → 读关联 Review；②无匹配才全局 glob |
| P0 | `Handoff` / `handoff` / `交接` | **二段式**: ①先读 `.vscode/plans/index.md` 定位 plan → 读 handoff；②无匹配才全局 glob |
| P0 | `add-route` / `执行路线图` | **二段式**: ①先读 `.vscode/plans/index.md` 定位 plan → 读 add-route；②调 `check_add_route_status`；③无匹配才全局 glob |
...
| P1 | `review-implementation` | 查 `.vscode/reviews/` 下 `*-review-implementation*.md` |
| P1 | `review-runtime` | 查 `.vscode/reviews/` 下 `*-review-runtime*.md` |
