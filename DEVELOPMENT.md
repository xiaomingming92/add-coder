...
- [十九、架构适应度函数（fitness function）](#十九架构适应度函数fitness-function)
  - [19.1 一条合格的适应度函数](#191-一条合格的适应度函数)
  - [19.2 四类反模式与对应判据](#192-四类反模式与对应判据)
  - [19.3 执行卡位](#193-执行卡位)
  - [19.4 与单一真源、caijuehub 的关系](#194-与单一真源caijuehub-的关系)
- [关联文档](#关联文档)
...
真源要被守住，先把真源清单落成**注册表**（谁是真源、谁是产物、谁是谁的上游），再定义比对口径——否则「同步了」「没漂移」都只是感觉：

| 口径 | 规则 | 反例（必须报） |
|------|------|----------------|
| **判据用内容哈希** | 是否漂移看内容哈希（SHA-256）；mtime 只作「要不要重算」的触发 | 用 mtime 判同步——`touch` 一下真源就误报漂移，或反过来漏报 |
| **混合区段按区段比对** | 副本里既有生成区段又有手写区段时（`*.strategy.ts` 生成头、hook 产物烘焙块、带 `GENERATED` 标记的模板），只比对生成区段 | 整文件哈希——手写区段的合法改动被误判「漂移」，最后演变成整文件覆盖、手写丢失 |
| **生成物新鲜度看输入指纹** | 生成物旁记输入指纹（源清单 + 各源哈希 + 生成器版本）；「新鲜」= 当前输入指纹 == 记录指纹 | 只看生成物存在 / 只看生成时间——改了真源没重跑生成器，仍报「新鲜」 |
| **漏登记即失败** | 消费者按注册表驱动；未登记的文档类型 / 源显式失败（同 §16.2 `UnknownValidatorTypeError` 口径），不回落「通用校验」 | 漏登记 → 静默跳过 → 假阴性「无漂移」 |

两条派生要求：**源清单要闭包到上游输入**（只登记中间产物 = 把产物当源，见 §19.1 前科②）；**比对要在明确作用域内做**（作用域不写清，跨轮同名会被误判成重复或漂移）。把口径挂成可执行门禁的做法见 §十九。
...
## 十九、架构适应度函数（fitness function）

> 命名借自演进式架构：**适应度函数 = 把一条架构约定写成可执行、会失败的检查**，挂到能拦住人的卡位上。
> 真源与比对口径见 §三；规则真源与生成链见 [docs/caijuehub.md](./docs/caijuehub.md)；文档形式判定见 §十六。
> 取证来源：跨项目审计记录（脱敏，只保留机制与教训）。

### 19.1 一条合格的适应度函数

| 要素 | 要求 | 反例 |
|------|------|------|
| **可执行** | 一个命令或一个 hook 入口，退出码即结论 | 只在文档 / PR 模板里写约定 |
| **会失败** | 违反时非 0 退出或 blocking 卡位拦截，失败信息点名文件 / 卡位 / 行 | `console.warn` 完事，退出码恒 0 |
| **可复现** | 同树同输入 → 同结论（判据用内容哈希，不看 mtime、不看「我刚跑过」） | 依赖上一次执行的人肉记忆 |
| **判据三件套** | ① 生产点（非测试代码、运行时可达）② 注册表条目（真源清单中登记）③ 运行时相位（日志 / 审计取证） | 只查文件存在性——「生成了」≠「接线了」 |

三条合起来才是「接线」的可证伪定义：文件存在（静态）→ 被注册表驱动（可达）→ 运行时真的走到（相位）。缺任一条，都会长出「写了不接线」的产物。

**两条实测前科（机制 → 教训）**

| 前科 | 机制 | 教训 |
|------|------|------|
| **恒绿桩** | 唯一执行入口把 readiness 传成 `() => ({ ready: true })`：依赖声明（`dependsOn` / `requiredReadiness` / `onStale`）全部形同虚设，门禁永远绿 | 判据必须在生产点取真值；门禁的默认值应为**失败**，且要有「判据本身被替换」的检查 |
| **把产物当源** | 漂移检查的源清单只登记产物、未覆盖上游采集源：上游改了、没重跑采集，仍报「无漂移」 | 源清单必须闭包到上游输入；「无漂移」的结论只在源清单完备时成立 |

### 19.2 四类反模式与对应判据

| 反模式 | 症状 | 可执行判据 |
|--------|------|-----------|
| **写了不接线** | 新模块 / 新 hook 产物生成出来了，没人 import、没有入口触发 | 判据三件套（§19.1） |
| **基类不派生** | 绕开契约层自己实现一遍；或继承后不覆写必覆写点，靠默认行为蒙混 | 必覆写点用类型收紧（抽象成员 / 必填参数 / 判别联合），编译期失败 |
| **模板玩法造轮子** | 同一能力出现第二个平行实现，行为随时间分叉 | 同一能力只允许一处定义 + 平行实现黑名单（登记进注册表） |
| **故意狭隘实现** | 只覆盖「能过门禁的样例」：边界矩阵只填一格、硬编码哨兵绕过判据 | 边界矩阵逐格断言 + 硬编码哨兵扫描（哨兵常量 / 短路分支进检查） |

四类是同一个根因的四种表现：把「看起来做完」当「做完」。适应度函数的收益，是把验收从人肉读 diff 挪到可复现的失败上。

### 19.3 执行卡位

| 卡位 | 执行物 | 口径 |
|------|--------|------|
| 写入前 | hook `doc-format-guard` / `pre-tool-use`（§十六） | blocking（不合规文档写不进去） |
| 提交前 | `npm run generate:check` · `npm run contributors:check` · `npx tsx scripts/validate-docs.ts --strict` | 非 0 退出 |
| 发布前 | `npm run release:preflight` | 非 0 退出 |
| 收尾 / 封口 | `stop-check` / review-checklist（§16.4） | blocking |
| 巡检 | `npx tsx scripts/validate-docs.ts --hook <Hook>` 复现任一卡位口径 | advisory；`--strict` 才非 0 退出 |

判别口诀：**能拦住人的卡位才叫门禁**；拦不住又天天红的检查叫噪音。fail-open 的告警（如 §十八 的产物-进程新鲜度告警，退出码恒 0）是有意为之——文档里必须写明它是提示、不冒充门禁。

### 19.4 与单一真源、caijuehub 的关系

```
caijuehub TOML 真源 ──npm run generate──▶ *.strategy.ts 生成物 ──import──▶ 消费方
        ▲                                        ▲
        │ 检查：改了真源没重跑生成器？             │ 检查：生成区段被手改？
        └────────────────────────────────────────┘
                  架构适应度函数（可执行、非 0 退出）
```

- **单一真源**回答「正确答案在哪」（§三）；**适应度函数**回答「怎么知道现在这份是对的」。前者是声明，后者是判据；缺判据的真源会腐化成文档里的一句约定。
- **caijuehub** 提供规则侧真源与生成链（TOML → 生成物 → 消费），适应度函数提供生成物侧的接线 / 漂移 / 手改检查——两者接上才是闭环；否则「改规则不改代码」只是投产快，不知道有没有投错。
- 现有家族（可跑）：`npm run generate:check` · `npm run contributors:check` · `npm run release:preflight` · `scripts/validate-docs.ts --strict` · §十六 卡位矩阵 · §十八 新鲜度四态。
