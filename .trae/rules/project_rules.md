...
SKILL 文件位于 `.trae/skills/add-paradigm/SKILL.md`，包含 10 个阶段（Step 0 - Step 9），每个阶段包含若干子步骤：
...
- ✅ `.trae/plans/add-coder-agrisynapse-integration-plan-v1.md`（.trae 内文件）
...
|---------|:---:|------|
| Plan（标准版/精简版） | `PLAN` | `.trae/plans/{YYYY-MM}/{DD}/` |
| Review（方案/实现） | `PLAN_REVIEW` | `.trae/reviews/` |
| Runtime Review | 不需要 | `[T]` 全部通过后直接写入 `.trae/reviews/` |
| Spec / Tasks / Checklist | 不需要 | 直接写入 `.trae/specs/` |
| Handoff | 不需要 | 直接写入 `.trae/plans/` |
...
**Plan、方案 Review 和实现 Review 在写入正式文件前，必须经过 HITL 审批。** 审批通过后，`update_hitl` 更新数据库与 `*.hitl.md` 状态，并在 `.trae/hitl/` 生成 `.tongyi-{planName}` 哨兵。pre-tool-use hook 对 Plan/方案 Review 执行哨兵门禁；实现 Review 由 `add-paradigm` Step 3.5 与 `status_hitl` 门禁约束。
...
> 附录 A（add-coder ADD-0.3 AuditCallback 实现，含 LangChain `BaseCallbackHandler` 模式）已移至技术栈 profile：`.trae/rules/profiles/webapp-profile.md` §附录 A。未设置 webapp 技术栈时，该附录内容不构成约束。
