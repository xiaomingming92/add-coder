...
3. **命名规范**：`{项目名}-{功能名}-plan-v1.md` → `.trae/plans/{YYYY-MM}/{DD}/`（按当天日期创建子目录）
...
**此外，ADD 工作流的核心产物由 `.trae/templates/` 下的 13 个模板定义**，这些模板不是参考资料，而是每次变更必须产出的文档骨架。分析变更影响范围时，必须同步确认需要创建/更新哪些模板产物：
...
> 4. TONGYI 通过后，生成完整 Review 写入 `.trae/reviews/`
...
**命名**：`{需求域名}-{核心内容}-add-route-v1.md` → `.trae/plans/{YYYY-MM}/{DD}/`（与 Plan 同目录）
...
**什么时候触发**：
- Plan Review 已生成（`.trae/reviews/{需求域名}-*review-v{n}.md` 存在）
...
1. **判定输入**：当前 adapter 自己的 `.trae/plans/`。各 adapter 的 Plan/Spec/Route/Review/HITL 相互独立，禁止跨 adapter fallback
...
> **全局文档编辑规则（A.0）**：修改 Plan、Spec、Handoff、tasks、checklist 中已有内容时，MUST 遵循增量修订格式——旧内容 `~~删除线~~` → 新内容 `[日期 修订: 原因]`，禁止直接覆盖。详见 `.trae/rules/project_rules.md` §文档增量修订规则。
...
| 文档类型 | 命名规则 | 示例 | 存放位置 |
|---------|---------|------|---------|
| 开发任务（specs 三元组） | `项目名-任务名/` | `add-coder-response-strategy/` | `.trae/specs/` |
| review 文件 | `项目名-任务名-round{N}-review.md` | `add-coder-response-strategy-round2-review.md` | `.trae/reviews/` |
| handoff 文件 | `项目名-需求名-handoff.md` | `add-coder-co-agent-handoff.md` | `.trae/plans/{YYYY-MM}/{DD}/`（与 Plan 同目录） |
...
检查 `.trae/specs/{task}/checklist.md` 中的所有检查项：
...
读取 `review-implementation-template.md`，将实现审查发现整理为 `dimensions`，调用 `create_hitl({ planName: "{name}-review-implementation", type: "PLAN_REVIEW", dimensions })`。`create_hitl` 自动生成 `*.hitl.md` 审批提案并写入 HITL 记录；人类审核后调用 `update_hitl` 完成拍板。通过 `status_hitl` 确认状态为 `TONGYI` 后，方可逐条展开详细分析并写入 `.trae/reviews/`；若为 `BOHUI`，修订发现后调用 `create_hitl` 新建 round。
...
2. 复制为 `.trae/reviews/{project}-review-runtime.md`
3. 替换占位符（标题、关联文档路径）
...
> **收敛确认后、handoff 生成前，MUST 记录 ROUND_CLOSED。** ADD-7 原则见 `.trae/rules/project_rules.md` §ADD-7。
...
5. **写入位置**：`{项目名}-{需求名}-handoff.md` → `.trae/plans/`
...
读取 `.trae/templates/report-handoff-template.md`，按模板格式在 handoff 中追加 Report Closure 章节。
