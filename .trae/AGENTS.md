...
> 本文件由 add-coder 模板为项目 `add-coder` 渲染生成（真源 `templates/core/AGENTS.md`），占位符按项目的 `.trae` 展开。
...
如本项目与其他项目共享宿主端口，**先查事实源登记表**再占用；项目内端口约定统一记录在 `.trae/rules/` 或项目 `docs/ports.md`，禁止在本文件里维护第二份端口清单。
