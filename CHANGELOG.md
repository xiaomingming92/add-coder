...
> 现只保留该批次条目（`check_spec_sync` 三项修复 + `[W]` 接线判据回灌）；当日（09-21）的条目
> —— 多宿主三端文档 / MCP Apps 扩展协商 / Plan 生命周期关闭 / status 宿主自检 / status 文案加固 /
> VS Code 折叠（Issue #21）/ HITL widget 缓存键 —— 已按发布窗口**全部归入本段**（该是谁的就归谁）。
