// templates/adapters/codex/hooks/pre-tool-use.ts
...
// templates/adapters/codex/hooks/pre-tool-use.ts
var CodexPreToolUseGuard = class extends PreToolUseGuard {
...
`);
    ...
    writeHookEvent("pre-tool-use", "deny", `${toolName2} ${filePath}`, `HITL \u672A\u540C\u610F: ${tongyiMarker}`, this.planKeyword, this.planStatus);
    return 2;
  }
  /** ③ apply_patch: 解析 patch 路径 → guardFilePath 逐文件 + markDevAction（codex 特有） */
  onOtherTool(input2, toolName2) {
    if (toolName2 !== "apply_patch") return 0;
    let patchCommand = "";
    if (input2.trim() !== "") {
      try {
...
var guard2 = new CodexPreToolUseGuard(PROJECT_DIR, MAGIC_DIR);
if (toolName === "Bash") {
