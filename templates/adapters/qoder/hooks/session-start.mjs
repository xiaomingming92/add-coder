// templates/adapters/qoder/hooks/session-start.ts
...
var SessionStartGuard = class {
  ...
  constructor(projectDir) {
    ...
    const inferred2 = tryResolveMagicDir();
    if (inferred2 && !process.env.MAGIC_DIR) {
      process.env.MAGIC_DIR = inferred2;
    }
...
// templates/adapters/qoder/hooks/session-start.ts
var PROJECT_DIR = injectProjectDir();
...
var QoderSessionStartGuard = class extends SessionStartGuard {
  ...
  emitState(state) {
    ...
    addCtx += "\n[\u4EE3\u529E] \u5982\u6709\u672A\u52A0\u8F7D\u7684 IDE \u4EE3\u529E\u6E05\u5355\uFF0C\u8BF7\u4ECE tasks.md \xA7IDE JSON \u5237\u65B0 TodoWrite";
  ...
  /** ③ 代办已并入状态段 */
  emitTodoReminder(_state) {
  }
  /** ④ HITL 待审批: JSON 注入（对齐 bash find *.hitl.md -mtime -7） */
  emitHitlPending() {
    const plansDir = join4(PROJECT_DIR, MAGIC_DIR, "plans");
    if (MAGIC_DIR !== "" && existsSync4(plansDir)) {
      const weekAgo = Date.now() - 7 * 24 * 3600 * 1e3;
      let hitlCount = 0;
      try {
        hitlCount = readdirSync2(plansDir).filter((f) => f.endsWith(".hitl.md")).filter((f) => {
          try {
            return statSync2(join4(plansDir, f)).mtimeMs >= weekAgo;
          } catch {
            return false;
          }
        }).length;
