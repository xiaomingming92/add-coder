// permission-gate.ts — PermissionRequest 入口（Trae 版，Task 6.1 继承体系）
// 继承 core PermissionGateRouter，命名子类 TraePermissionGateRouter:
...
class TraePermissionGateRouter extends PermissionGateRouter {
  // 当前无 override（`// empty` 回退 + 高风险工具提示与 core 基线一致）；命名子类承载端身份 + 未来演进位
}

const input = readHookInput()
process.exit(new TraePermissionGateRouter().run(input))
