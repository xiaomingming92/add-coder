...
- 本 handoff 是新对话的入口索引。具体实现细节以 `{{magicDir}}/specs/add-coder-add-coder-npm-package/spec.md`、`tasks.md`、`checklist.md` 为准。
...
3. read "{{magicDir}}/specs/add-coder-add-coder-npm-package/spec.md"
4. read "{{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md"（第1轮）
5. read "{{magicDir}}/specs/add-coder-add-coder-npm-package/checklist.md"（一、二、四）
```
...
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/spec.md`
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md`（第1轮）
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/checklist.md`（一、二、四）
...
| `templates/` 下 15 个 `.md` 模板 | 修改 | `add-coder-*` → `{{projectName}}-*` |
| `skills/` 下 2 个 SKILL.md | 修改 | `/home/xmm/` → `{{projectRoot}}/` |
| `scripts/mcp-server.ts` + `add-coder-mcp-server.ts` | 修改 | 数据库密码 → `process.env.DATABASE_URL` |
| `{{magicDir}}/settings.json` / `{{magicDir}}/mcp.json` | 修改 | 硬编码路径 → 占位符 |
| `{{magicDir}}/hooks/` 下 14 个 `.sh` + `lib/` | 修改 | 项目名提取逻辑 |
| `.vscode/` 下 4 个文件 | 修改 | MCP 路径 → `{{projectRoot}}` |
| `reports/` 下 7 个文件 | 修改 | 硬编码 → `{{projectName}}` |
| `packages/add-coder/src/core/renderer.ts` | 新建 | 核心渲染器 |

### 关键契约

- 模板变量语法：`{{projectName}}` 双花括号，禁止 `${var}`
...
2. read "{{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md"（第2轮）
```
...
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md`（第2轮）
...
| `templates/{{magicDir}}/hooks/` | `templates/adapters/qoder/hooks/` | 迁移 |
| `templates/{{magicDir}}/settings.json` | `templates/adapters/qoder/settings.json` | 迁移 |
| `templates/{{magicDir}}/mcp.json` | `templates/adapters/qoder/mcp.json` | 迁移 |
...
```
1. query_audit_logs({ planKeyword: "add-coder-npm-package" })
2. read "{{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md"（第3轮）
3. read "{{magicDir}}/specs/add-coder-add-coder-npm-package/checklist.md"（三）
```
...
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md`（第3轮）
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/checklist.md`（三）
...
```
1. query_audit_logs({ planKeyword: "add-coder-npm-package" })
2. read "{{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md"（第4轮）
3. read "{{magicDir}}/specs/add-coder-add-coder-npm-package/checklist.md"（五）
```
...
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md`（第4轮）
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/checklist.md`（五）
...
```
1. query_audit_logs({ planKeyword: "add-coder-npm-package" })
2. read "{{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md"（第5轮）
3. read "{{magicDir}}/specs/add-coder-add-coder-npm-package/checklist.md"（五、六、八）
```
...
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md`（第5轮）
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/checklist.md`（五、六、八）
...
```
1. query_audit_logs({ planKeyword: "add-coder-npm-package" })
2. read "{{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md"（第6轮）
3. read "{{magicDir}}/specs/add-coder-add-coder-npm-package/checklist.md"（七）
...
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/spec.md`（Requirement: CaijueHub 裁决层）
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md`（第6轮）
- `{{magicDir}}/specs/add-coder-add-coder-npm-package/checklist.md`（七）
...
| 模板部署策略 | 仅 `.add/` | `{{magicDir}}/` `.claude/` 同时部署 core 内容 | IDE 只认自身 magic path，需三者各自完整 |
...
| Review v2 回流 | 无 | `{{magicDir}}/reviews/add-coder-add-coder-npm-package-review-v2.md` | 代码实现评审：2 条 P0（spawnSync catch 失能、ask 重复定义）在 Plan §4.8 回流，P0-1 已补充至 Task 1.0 验收项 |
...
上游 [第1轮~第N-1轮] 已完成。
先读 {{magicDir}}/plans/2026-07/08/add-coder-add-coder-npm-package-handoff-v1.md 的 <第N轮> 章节。

## 启动步骤（按顺序）

1. 执行 session-init SKILL
2. 执行 add-paradigm SKILL
3. 读 {{magicDir}}/specs/add-coder-add-coder-npm-package/spec.md
4. 读 {{magicDir}}/specs/add-coder-add-coder-npm-package/tasks.md（第N轮）
5. 读 {{magicDir}}/specs/add-coder-add-coder-npm-package/checklist.md
