...
> **完整词汇表已独立维护**：`{{magicDir}}/vocabulary/add-governance-vocabulary.md`
...
> 完整结构化映射：`{{magicDir}}/rules/theory-practice-map.toml`（可被脚本解析验证）
...
SKILL 文件位于 `{{magicDir}}/skills/session-init/SKILL.md`，包含 4 个步骤：
...
SKILL 文件位于 `{{magicDir}}/skills/add-paradigm/SKILL.md`，包含 10 个阶段（Step 0 - Step 9），每个阶段包含若干子步骤：
...
| `{{magicDir}}/plans/` | 需求方案（plan）+ 任务拆分（execution）+ 轮间交接手册（handoff） | 开发内部 |
| `{{magicDir}}/reviews/` | 方案评审 + 逐轮 spec 评审 | 开发内部 |
| `{{magicDir}}/specs/` | 每轮 spec + tasks + checklist（三件套） | 开发内部 |
| `{{magicDir}}/templates/` | 文档模板（plan/spec/tasks/checklist/review） | 开发内部 |
| `{{magicDir}}/rules/` | 项目规则文件 | 开发内部 |
| `{{magicDir}}/skills/` | SKILL 行为定义 | 开发内部 |
...
```
需求理解 + 任务拆分 → {{magicDir}}/plans/      （plan + execution + handoff）
    ↓
Review（强制关卡）    → {{magicDir}}/reviews/    （plan-review + roundN-spec-review）
    ↓
...
- 每轮的 Review 文件路径：`{{magicDir}}/reviews/{需求域名}-round{N}-{核心内容}-spec-review-v{版本}.md`

### 文档产出物模板

ADD 开发流程产生五类文档，模板文件位于 `{{magicDir}}/templates/`：

| 模板 | 文件路径 | 说明 |
|------|---------|------|
| Plan | `{{magicDir}}/templates/plan-template.md` | 元信息 + 背景目标 + 方案选型 + 架构设计 + 实施步骤 + 验收标准 + 关联文档 |
| Spec | `{{magicDir}}/templates/spec-template.md` | Why + What Changes + Impact + Requirements(WHEN/THEN) |
| Tasks | `{{magicDir}}/templates/tasks-template.md` | Phase → Task → SubTask 层级 |
| Checklist | `{{magicDir}}/templates/checklist-template.md` | Phase 检查项 + ADD 规则合规检查 |
| Review | `{{magicDir}}/templates/review-template.md` | 元信息 + 问题复现 + 方案对比 + 决策结论 + 影响评估 |
| Handoff（单轮） | `{{magicDir}}/templates/handoff-single-round-template.md` | 单轮变更 Handoff 基础格式（9 章节） |
| Handoff（多轮） | `{{magicDir}}/templates/handoff-multi-round-template.md` | 多轮原子事务 Handoff（全局结构 + 13 子章节 + 收敛规则 + 启动模板） |
...
| 5 | **spec 文件** | ✅ | `{{magicDir}}/specs/{spec-name}/spec.md` + `tasks.md` + `checklist.md` 三件套路径 |
...
AI 助手在**每次新对话启动时**，必须先执行 `{{magicDir}}/skills/session-init/SKILL.md`（SKILL-1 会话初始化），作为**不可跳过的前置操作**。

`session-init` SKILL 的 Step 1 要求调用 `query_audit_logs` 工具查询开发操作审计记录，以恢复开发上下文。

**执行顺序**（新对话启动时的首个操作）：
1. 定位并执行 `session-init` SKILL（`{{magicDir}}/skills/session-init/SKILL.md`）
...
MCP 服务器配置在 `{{magicDir}}/mcp.json`，通过 `npx tsx {{magicDir}}/scripts/mcp-server.ts` 启动。
