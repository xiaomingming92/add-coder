...
- **关联文档**:
  - ADD Route: `.claude/plans/2026-07/08/add-coder-add-coder-npm-package-add-route-v1.md` ✅
  - Handoff: `.claude/plans/2026-07/08/add-coder-add-coder-npm-package-handoff-v1.md` ✅
  - Review: `.claude/reviews/add-coder-add-coder-npm-package-review-v1.md` ✅
  - Review v2: `.claude/reviews/add-coder-add-coder-npm-package-review-v2.md` ✅ — 代码实现评审，2 条 P0（spawnSync catch 失能、ask 重复定义）
- **ADD-7 审计策略**:
...
5. **配置智能合并**：不覆盖用户已有的 `.claude/settings.json`、`.vscode/settings.json` 等配置
...
src/cli/commands/init.ts        ── 主命令
        ...
        ├── ① 检测 IDE 环境
        │     src/cli/detect.ts
        │     → 扫描 .claude/ .claude/ .vscode/ 存在性 → 确定 targetAdapter
...
> **部署策略**：`templates/core/` 内容复制到 `.add/`、`.claude/`、`.claude/` 三个目录。
> IDE 只认自身的 magic path（`.claude/` / `.claude/`），必须各自包含完整内容。
...
> 2. `src/cli/commands/init.ts`：core 文件写入 `.add/` `.claude/` `.claude/` 三个目标
...
|------|--------|-------|---------|
| Magic Path | `.claude/` | `.claude/` | `.vscode/` |
...
| Skills 加载 | `.claude/skills/` | `.claude/skills/` | 不支持 |
| Subagents 加载 | `.claude/agents/` | `.claude/agents/` | 不支持 |
...
| Writer 写入 | `{ path: ".claude/settings.json", existing: true }` | `{ decision: "skip", reason: "--yes" }` |
...
| 12 | Hook 配置 | `.claude/settings.json` | hook 脚本绝对路径 `/home/xmm/ai/add-coder/.claude/hooks/` | `/home/xmm/ai/add-coder/.claude/hooks/` | Init-time |
| 13 | MCP 配置 | `.claude/mcp.json` | `DATABASE_URL` 硬编码密码 | `process.env.DATABASE_URL`（无兜底值） | Runtime |
| 14 | 同步策略 | `.claude/sync-policy.json` | 无项目特定硬编码 | 检查确认 | — |
| 15 | Hook 脚本 | `.claude/hooks/` 下 14 个 `.sh` 文件 | `state-detect.sh` 中项目名提取逻辑、hook 脚本内部绝对路径 | 使用 `$CLAUDE_PROJECT_DIR` 或 `/home/xmm/ai/add-coder` 替代 | Init-time |
...
| `.claude/hooks/`（14 个 .sh + lib/） | `templates/adapters/qoder/hooks/` | Qoder 适配器 |
| `.claude/settings.json` | `templates/adapters/qoder/settings.json` | Qoder 适配器 |
| `.claude/mcp.json` | `templates/adapters/qoder/mcp.json` | Qoder 适配器 |
| `.claude/sync-policy.json` | `templates/adapters/qoder/sync-policy.json` | Qoder 适配器 |
...
- 从现有 `templates/.claude/` 迁移到 `templates/adapters/qoder/`，清理 hardcode
...
**验收**：`npx add-coder init --adapter qoder` 生成正确的 `.claude/` 目录
...
- [ ] 已有 `.claude/settings.json` 时 `init` 不覆盖，展示 diff 并交互确认
- [ ] `npx tsc --noEmit` 通过
...
| Handoff | `.claude/plans/2026-07/08/add-coder-add-coder-npm-package-handoff-v1.md` | ✅ |
| Review | `.claude/reviews/add-coder-add-coder-npm-package-review-v1.md` | ✅ |
| Spec | `.claude/specs/add-coder-add-coder-npm-package/spec.md` | ✅ |
| Tasks | `.claude/specs/add-coder-add-coder-npm-package/tasks.md` | ✅ |
| Checklist | `.claude/specs/add-coder-add-coder-npm-package/checklist.md` | ✅ |