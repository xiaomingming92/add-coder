...
读取 `.claude/plans/` 目录，按以下优先级定位 Plan 对应的 add-route 文件：
...
1. **读索引** — 读取 `.claude/plans/index.md`，按 planKeyword 匹配「主题」列，找到同主题的 `add-route` 类型行 → 直接获得文件路径。
...
| 检查项 | 方法 | 不通过 |
|--------|------|--------|
| add-route 存在性 | 递归搜索 `.claude/plans/`（含 `YYYY-MM/DD/` 子目录）确认 `*add-route*.md` 文件存在 | 不存在 → **BLOCKED** — 回退 Step 0.5 |
...
| gateway.md 发现可定位 | 确认 `.claude/reports/add-coder-runtime-report/gateway.md` 存在 | 缺失 → **FAIL** |
...
| 检查项 | 方法 | 判定 |
|--------|------|------|
| Plan 文件存在 | 确认 `.claude/plans/` 下（含日期子目录）有对应的 `-plan-v*.md` | 缺失 → **BLOCKED** |
| Specs 三元组就绪 | 确认 `spec.md` + `tasks.md` + `checklist.md` 存在 | 缺失 → **BLOCKED** |
| Review 文件存在 | 确认 `.claude/reviews/` 下有对应的 review 文件 | 缺失 → **BLOCKED** |
| add-route 已生成 | 确认 `.claude/plans/` 下（含日期子目录）有 `*add-route*.md` | 缺失 → **BLOCKED** |
