...
3. **命名规范**：`{项目名}-{功能名}-plan-v1.md` → `.claude/plans/{YYYY-MM}/{DD}/`（按当天日期创建子目录）
...
> 4. TONGYI 通过后，生成完整 Review 写入 `.claude/reviews/`
...
1. **判定输入**：当前 adapter 自己的 `.claude/plans/`。各 adapter 的 Plan/Spec/Route/Review/HITL 相互独立，禁止跨 adapter fallback
...
> **全局文档编辑规则（A.0）**：修改 Plan、Spec、Handoff、tasks、checklist 中已有内容时，MUST 遵循增量修订格式——旧内容 `~~删除线~~` → 新内容 `[日期 修订: 原因]`，禁止直接覆盖。详见 `.claude/rules/project_rules.md` §文档增量修订规则。
...
读取 `review-implementation-template.md`，将实现审查发现整理为 `dimensions`，调用 `create_hitl({ planName: "{name}-review-implementation", type: "PLAN_REVIEW", dimensions })`。`create_hitl` 自动生成 `*.hitl.md` 审批提案并写入 HITL 记录；人类审核后调用 `update_hitl` 完成拍板。通过 `status_hitl` 确认状态为 `TONGYI` 后，方可逐条展开详细分析并写入 `.claude/reviews/`；若为 `BOHUI`，修订发现后调用 `create_hitl` 新建 round。
...
> **收敛确认后、handoff 生成前，MUST 记录 ROUND_CLOSED。** ADD-7 原则见 `.claude/rules/project_rules.md` §ADD-7。
