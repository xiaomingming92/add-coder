...
var PostToolFailureRouter = class {
  ...
  run(input) {
    ...
    const toolName = empty ? this.emptyUsesFallback() ? this.fallbackToolName() : "" : fieldOr(input, "tool_name", this.fallbackToolName());
    const error = empty ? this.emptyUsesFallback() ? this.fallbackError() : "" : fieldOr(input, "error", this.fallbackError());
...
function runPostToolFailure() {
  process.exit(new PostToolFailureRouter().run(readHookInput()));
}

// templates/core/hooks/post-tool-failure.ts
runPostToolFailure();
