...
SKILL 文件位于 `.add/skills/session-init/SKILL.md`，包含 4 个步骤：
...
SKILL 文件位于 `.add/skills/add-paradigm/SKILL.md`，包含 10 个阶段（Step 0 - Step 9），每个阶段包含若干子步骤：
...
**此外，ADD 工作流的核心产物由 `.add/templates/` 下的 13 个模板定义**，这些模板不是参考资料，而是每次变更必须产出的文档骨架。分析变更影响范围时，必须同步确认需要创建/更新哪些模板产物：
...
- ✅ `.add/plans/add-coder-agrisynapse-integration-plan-v1.md`（.add 内文件）
...
| `.add/plans/` | 需求方案（plan）+ 任务拆分（execution）+ 轮间交接手册（handoff） | 开发内部 |
| `.add/reviews/` | 方案评审 + 逐轮 spec 评审 | 开发内部 |
| `.add/specs/` | 每轮 spec + tasks + checklist（三件套） | 开发内部 |
| `.add/templates/` | 文档模板（plan/spec/tasks/checklist/review） | 开发内部 |
| `.add/rules/` | 项目规则文件 | 开发内部 |
| `.add/skills/` | SKILL 行为定义 | 开发内部 |
| `.add/scripts/` | 工具脚本 + MCP 服务器 | 开发内部 |
...
```
需求理解 + 任务拆分 → .add/plans/      （plan + execution + handoff）
    ↓
Review（强制关卡）    → .add/reviews/    （plan-review + roundN-spec-review）
    ↓
Spec 执行              → .add/specs/       （三件套：spec + tasks + checklist）
```
...
- 每轮的 Review 文件路径：`.add/reviews/{需求域名}-round{N}-{核心内容}-spec-review-v{版本}.md`

### 文档产出物模板

ADD 开发流程产生五类文档，模板文件位于 `.add/templates/`：

| 模板 | 文件路径 | 说明 |
|------|---------|------|
| Plan | `.add/templates/plan-template.md` | 元信息 + 背景目标 + 方案选型 + 架构设计 + 实施步骤 + 验收标准 + 关联文档 |
| Spec | `.add/templates/spec-template.md` | Why + What Changes + Impact + Requirements(WHEN/THEN) |
| Tasks | `.add/templates/tasks-template.md` | Phase → Task → SubTask 层级 |
| Checklist | `.add/templates/checklist-template.md` | Phase 检查项 + ADD 规则合规检查 |
| Review | `.add/templates/review-template.md` | 元信息 + 问题复现 + 方案对比 + 决策结论 + 影响评估 |
| Handoff（单轮） | `.add/templates/handoff-single-round-template.md` | 单轮变更 Handoff 基础格式（9 章节） |
| Handoff（多轮） | `.add/templates/handoff-multi-round-template.md` | 多轮原子事务 Handoff（全局结构 + 13 子章节 + 收敛规则 + 启动模板） |
...
| 5 | **spec 文件** | ✅ | `.add/specs/{spec-name}/spec.md` + `tasks.md` + `checklist.md` 三件套路径 |
...
|---------|:---:|------|
| Plan（标准版/精简版） | `PLAN` | `.add/plans/{YYYY-MM}/{DD}/` |
| Review（方案/实现） | `PLAN_REVIEW` | `.add/reviews/` |
| Runtime Review | 不需要 | `[T]` 全部通过后直接写入 `.add/reviews/` |
| Spec / Tasks / Checklist | 不需要 | 直接写入 `.add/specs/` |
| Handoff | 不需要 | 直接写入 `.add/plans/` |
...
**Plan、方案 Review 和实现 Review 在写入正式文件前，必须经过 HITL 审批。** 审批通过后，`update_hitl` 更新数据库与 `*.hitl.md` 状态，并在 `.add/hitl/` 生成 `.tongyi-{planName}` 哨兵。pre-tool-use hook 对 Plan/方案 Review 执行哨兵门禁；实现 Review 由 `add-paradigm` Step 3.5 与 `status_hitl` 门禁约束。
...
AI 助手在**每次新对话启动时**，必须先执行 `.add/skills/session-init/SKILL.md`（SKILL-1 会话初始化），作为**不可跳过的前置操作**。

`session-init` SKILL 的 Step 1 要求调用 `query_audit_logs` 工具查询开发操作审计记录，以恢复开发上下文。

**执行顺序**（新对话启动时的首个操作）：
1. 定位并执行 `session-init` SKILL（`.add/skills/session-init/SKILL.md`）
...
MCP 服务器配置在 `.add/mcp.json`，通过 `npx tsx .add/scripts/mcp-server.ts` 启动。
