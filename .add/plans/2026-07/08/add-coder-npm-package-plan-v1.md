...
- **Plan 名称**: add-coder-npm-package-v1
- **启动时间**: 2026-07-08T00:00:00+08:00
- **主导 AI**: Qoder (Claude 4)
- **关联文档**:
  - ADD Route: `.add/plans/2026-07/08/add-coder-npm-package-add-route-v1.md` ✅
  - Handoff: `.add/plans/2026-07/08/add-coder-npm-package-handoff-v1.md` ✅
  - Review: `.add/reviews/add-coder-npm-package-review-v1.md` ✅
  - Review v2: `.add/reviews/add-coder-npm-package-review-v2.md` ✅ — 代码实现评审，2 条 P0（spawnSync catch 失能、ask 重复定义）
- **ADD-7 审计策略**:
...
5. **配置智能合并**：不覆盖用户已有的 `.add/settings.json`、`.vscode/settings.json` 等配置
...
src/cli/commands/init.ts        ── 主命令
        ...
        ├── ① 检测 IDE 环境
        │     src/cli/detect.ts
        │     → 扫描 .add/ .claude/ .vscode/ 存在性 → 确定 targetAdapter
...
> **部署策略**：`templates/core/` 内容复制到 `.add/`、`.add/`、`.claude/` 三个目录。
> IDE 只认自身的 magic path（`.add/` / `.claude/`），必须各自包含完整内容。
...
> 2. `src/cli/commands/init.ts`：core 文件写入 `.add/` `.add/` `.claude/` 三个目标
...
| Skills 加载 | `.claude/skills/` | `.add/skills/` | 不支持 |
| Subagents 加载 | `.claude/agents/` | `.add/agents/` | 不支持 |
...
| Writer 写入 | `{ path: ".add/settings.json", existing: true }` | `{ decision: "skip", reason: "--yes" }` |
...
|---|--------|------|----------|------|---------|
| 1 | 模板 | 15 个 `.md` 模板（plan/spec/tasks/checklist/handoff/review/add-route 等） | `docs/大田精准耕播智能决策系统/`、`*` 文件名示例 | `docs/`、`add-coder-*` | Init-time |
| 2 | 模板 Schema | 15 个 `.schema.json` | 无硬编码（结构定义） | 无需修改 | — |
| 3 | Skills | `skills/add-paradigm/SKILL.md` | `docs/大田精准耕播智能决策系统/`、`/home/xmm/ai/add-coder/`、`response-strategy/` 示例 | `docs/`、`/home/xmm/ai/add-coder/`、`add-coder-*` | Init-time |
| 4 | Skills | `skills/session-init/SKILL.md` | `review-runtime.md` 示例 | `add-coder-review-runtime.md` | Init-time |
...
| 12 | Hook 配置 | `.add/settings.json` | hook 脚本绝对路径 `/home/xmm/ai/add-coder/.add/hooks/` | `/home/xmm/ai/add-coder/.add/hooks/` | Init-time |
| 13 | MCP 配置 | `.add/mcp.json` | `DATABASE_URL` 硬编码密码 | `process.env.DATABASE_URL`（无兜底值） | Runtime |
| 14 | 同步策略 | `.add/sync-policy.json` | 无项目特定硬编码 | 检查确认 | — |
| 15 | Hook 脚本 | `.add/hooks/` 下 14 个 `.sh` 文件 | `state-detect.sh` 中项目名提取逻辑、hook 脚本内部绝对路径 | 使用 `$CLAUDE_PROJECT_DIR` 或 `/home/xmm/ai/add-coder` 替代 | Init-time |
...
| 23 | Grounding 文档 | `docs/01-架构/《ADD开发工作路径与文档协同规范》.md` | `docs/大田精准耕播智能决策系统/`、`*` 文件名示例（15 处） | `docs/`、`add-coder-*` | Init-time |
...
| `.add/hooks/`（14 个 .sh + lib/） | `templates/adapters/qoder/hooks/` | Qoder 适配器 |
| `.add/settings.json` | `templates/adapters/qoder/settings.json` | Qoder 适配器 |
| `.add/mcp.json` | `templates/adapters/qoder/mcp.json` | Qoder 适配器 |
| `.add/sync-policy.json` | `templates/adapters/qoder/sync-policy.json` | Qoder 适配器 |
...
- 从现有 `templates/.add/` 迁移到 `templates/adapters/qoder/`，清理 hardcode
...
**验收**：`npx add-coder init --adapter qoder` 生成正确的 `.add/` 目录
...
- [ ] 已有 `.add/settings.json` 时 `init` 不覆盖，展示 diff 并交互确认
- [ ] `npx tsc --noEmit` 通过
...
| Handoff | `.add/plans/2026-07/08/add-coder-npm-package-handoff-v1.md` | ✅ |
| Review | `.add/reviews/add-coder-npm-package-review-v1.md` | ✅ |
| Spec | `.add/specs/add-coder-npm-package/spec.md` | ✅ |
| Tasks | `.add/specs/add-coder-npm-package/tasks.md` | ✅ |
| Checklist | `.add/specs/add-coder-npm-package/checklist.md` | ✅ |