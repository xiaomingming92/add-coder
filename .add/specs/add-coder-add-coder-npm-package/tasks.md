...
- [x] Task 1.1: 清理硬编码 + 参数化 core 模板 — 验证: `grep -r "farm.agent\|farm_secure_pass\|大田精准\|/home/xmm\|/Users/milkytea" templates/` 返回空
  ...
  - [x] `.add/settings.json`：hook 脚本绝对路径 → `/home/xmm/ai/add-coder/.add/hooks/`
  - [x] `.add/mcp.json`：`DATABASE_URL` 硬编码密码 → `process.env.DATABASE_URL`
  - [x] `.add/sync-policy.json`：检查确认
  - [x] `.add/hooks/` 下 14 个 `.sh` + `lib/`：项目名提取逻辑 → 使用 `$CLAUDE_PROJECT_DIR` 或 `/home/xmm/ai/add-coder`
...
- [x] Task 2.0: 模板目录重组 + 适配器架构搭建 — 验证: 新目录结构符合 §3.2，`find templates/ -name "*.ts" | wc -l` 返回 0
  ...
  - [x] 迁移 `.add/hooks/`（14 个 .sh + lib/） → `templates/adapters/qoder/hooks/`
  - [x] 迁移 `.add/settings.json` → `templates/adapters/qoder/settings.json`
  - [x] 迁移 `.add/mcp.json` → `templates/adapters/qoder/mcp.json`
  - [x] 迁移 `.add/sync-policy.json` → `templates/adapters/qoder/sync-policy.json`
...
- [x] Task 3.1: Qoder 适配器实现 — 验证: `npx add-coder init --adapter qoder` 生成正确的 `.add/` 目录
...
- [x] Task 4.1: CLI 重写 — 验证: `npx add-coder init` 在空白项目中成功生成完整 ADD 模板
  ...
  - [x] `src/cli/detect.ts`：IDE 环境检测（扫描 `.add/` `.claude/` `.vscode/` 存在性）
...
- [x] `npx add-coder init --adapter qoder` 生成 `.add/` 目录
