...
| 5433 | PostgreSQL ADD 配套库 | add-coder 配套容器第一顺位：`add-coder-add-postgres`（宿主 5432 之后从 5433 起，示例） | 🟡 示例 | `.env.development` `ADD_DATABASE_URL` |
| 5434 | PostgreSQL Atlas dev 库 | Atlas 常驻 dev 空库 `add-coder-add-dev`（可随时重置，示例） | 🟡 示例 | `.env.development` `ATLAS_DEV_URL` |
...
| 容器 | 端口 | 归属 |
|------|------|------|
| `add-coder-add-postgres` | 5433 | 本项目 ADD 治理库（分库引导，示例） |
| `add-coder-add-dev` | 5434 | 本项目 Atlas dev 空库（可随时重置，示例） |
