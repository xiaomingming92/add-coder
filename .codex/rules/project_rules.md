...
- ✅ `.codex/plans/add-coder-agrisynapse-integration-plan-v1.md`（.codex 内文件）
...
> 附录 A（add-coder ADD-0.3 AuditCallback 实现，含 LangChain `BaseCallbackHandler` 模式）已移至技术栈 profile：`.codex/rules/profiles/webapp-profile.md` §附录 A。未设置 webapp 技术栈时，该附录内容不构成约束。
